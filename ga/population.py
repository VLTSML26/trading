...
class Population(PopulationBase):
    ...
    def crossover(self, parents: np.array) -> np.array:
        ...
        for k in range(self.offspring_size[1]):
            ...
            # half of the second parent
            offspring[k, crossover_point:] = parents[parent2_idx, crossover_point:]
        return np.int32(offspring)

    def mutate(
        self, offspring_crossover: np.array, ind_params: Dict, pop_params: Dict
    ) -> np.array:
        for idx in range(offspring_crossover.shape[1]):
            ...
            offspring_crossover[idx][g] = offspring_crossover[idx][
                g
            ] + np.random.normal(
                pop_params[self._MUTATION_MEAN_KEY],
                pop_params[self._MUTATION_SD_KEY],
                1,
            )
        ...
        return np.int32(offspring_crossover)